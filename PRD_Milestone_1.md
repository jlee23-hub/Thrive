...
### 4.5 Shareworks API Contract Schema

```
GET /api/v1/participants/{participant_id}/grants
Response: {
...
```
GET /api/v1/participants/{participant_id}/vesting-schedule
Response: {
  "participant_id": "string",
  "vesting_events": [
    {
      ...
      "vesting_date": "ISO8601",
      "shares_to_vest": number,
      "vesting_status": "pending" | "vested" | "forfeited"
    }
  ]
}
```

### 4.6 Workday RaaS API Contract

```
GET /ccx/service/{tenant}/CRINT_Pave_Employee_Compensation_Records
Auth: OAuth 2.0 (Client Credentials)
Response: XML/JSON report with all mapped fields
Filters: Active workers, excludes Employee_Type in ("Intern", "Casual")

GET /ccx/service/{tenant}/CRINT_Pave_Compensation_History
Filters: Effective_Date >= (today - 2 years), Compensation_Change_Reason in ("Salary Change", "Bonus Change")

GET /ccx/service/{tenant}/CRINT_Pave_Employee_Eligible_Earnings
Filters: Current override period only
```
